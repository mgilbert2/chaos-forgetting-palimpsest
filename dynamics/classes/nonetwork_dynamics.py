...
class LearningRule:
    ...
    def f(self, x):
        return  self.scale * 0.5 * (2*self.qf-1.+np.tanh(self.betaf*(x-self.xf))) # tangent squashes inputs to a range between -1 and 1. It adds nonlinearity.
    # Larger 𝛽 means a sharper transition.
    def g(self, x):
...
class ConnectivityMatrix:
    ...
    def __init__(self,modelparams):
        ...
        if  self.tau == 'inf':
            self.p = modelparams['p'] # tirozzi and tsodyks model # Number of patterns
        else:
            #palimpsest setting
    ...
    def make_patterns(self, manual_patterns=None):
    #'''Generate patterns, or use provided manual patterns."
        if manual_patterns is not None:
             assert manual_patterns.shape[1] == self.N, "Manual patterns must match the number of neurons (N)."
             self.p = manual_patterns.shape[0]  # Update the number of patterns based on input
    ...
    def sparsity(self):
        '''scalling sparsity network's connectivity''' #  larger networks are sparser
        self.c = 1./self.N**0.3
        #self.c = 1./self.N**0.4
    ...
    def forgetting_kernel(self):
        ...
        # i(TIME STEP) corresponds to the indices generated by np.arange(0, self.p, 1)/ each index of i is a specific poattern 0 to p-1 (p=#of pats)
        # The forgetting kernel applies an exponential decay factor that depends on i Patterns stored earlier (smaller i)decay less than those stored later (larger i).       
        # self.tau_inv = forgetting rate 1/τ 
        # K is connectivity 
# "i" essentially tracks each pattern's position in time, influencing how much it is affected by forgetting
    def _make_pre_post_patterns(self):
        ...
        patterns_fr = self.make_patterns() # Generate patterns
        patterns_pre = self.myLR.g(patterns_fr) # Apply learning rule `g` to patterns (presynaptic)
        patterns_pre = np.einsum('ij,i->ij', patterns_pre, self.forget) # Apply forgetting kernel
        # each row of patterns_pre is scaled by the corresponding value from the forgetting kernel (self.forget[i])
        # weakens patterns that are "older" and keeps "newer" patterns stronger
        # the functions f and g represent Hebbian adjustments to the activity patterns

        patterns_post = self.myLR.f(patterns_fr) # Apply learning rule `f` to patterns (postsynaptic)
        print('Patterns created. N patterns:', self.p)
        return patterns_pre, patterns_post
# Pre-synaptic neurons send signals that weaken over time (forgetting kernel is applied).
# Post-synaptic neurons receive signals that have been adjusted by the f function but don’t weaken over time.

    def _make_indexes_connectivity(self):
        #number of entries different than zero
        self.N2bar = np.random.binomial(self.N * self.N, self.c) # Number of non-zero entries
        self.row_ind = np.random.randint(0, high = self.N, size = self.N2bar)  # Row indices for connections
        self.column_ind = np.random.randint(0, high = self.N, size = self.N2bar)  # Column indices for connections
        print('Structural connectivity created')
        # N2bar is number of connections n2 is total possibly connections and c is the sparsity 
        # Randomly selects row and column indices for the connections
    ...
    def connectivity_generalized_hebbian(self):
        ...
        # HEBB core operation
        # np.einsum function computes the element-wise product of pre and postsynaptic patterns and sums over the patterns
        connectivity = np.concatenate((connectivity, con_chunk),axis=0)	    
...
class NetworkDynamics:
    ...
    def __init__(self, parameters_values, connectivity):
        ...
        self.T = parameters_values['T'] # total sim time
        self.tau = parameters_values['tau'] # 20ms Time constant
        #input current
        self.Input = 0.
        ...
        self.un_0 = np.zeros(self.N) #the time 0 for the autocovariance Tracks neuron activity
        self.un_mean = np.zeros(self.N) # Tracks the mean activity of neurons over time.
    ...
    # calculates the rate of change of the neuron activity vector u at time t
    # 
    
    def _overlaps(self, rn, index): # rn is an array representing the current state of the network 
        ''' computing the overlaps'''
        overlap = (1./self.N) * np.einsum('ij,j->i',self.myLR.g(self.patterns_fr[index[0]:index[1], :]), rn) 
        # index[0]:index[1] selects the specific range of patterns to compare with rn
        # einsum: dot product between each transformed pattern (rows of the matrix) and rn, ->1D array, where each element corresponds to the overlap of a single pattern with rn
        # ij: The first array patterns has two dimensions: i (rows) and j (columns).
        return overlap
    
    def _autocovariance(self, un, t):
        '''computing the autocovariance'''
        if self.T_mean_start<=t: #autocovariance is only computed once the simulation has passed self.T_mean_start a time threshold
            N_t = (t-self.T_mean_start)/self.dt + 1 # N_t counts the number of time steps since self.T_mean_start
